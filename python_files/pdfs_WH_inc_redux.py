...
def MKPower(x1,order=1):
    ...
    for i in range(order):
        coef_power=r.RooRealVar("coef_power%d"%i,"coef_power%d"%i,i,-10,10)
...
def MKBernstein2(x):
    gc = []
    arglist = r.RooArgList()
    coef0 = r.RooRealVar("coef%d"%0,"coef%d"%0, 0.7 , 0.7-3*1.0, 0.7+3*1.0) 
    coef1 = r.RooRealVar("coef%d"%1,"coef%d"%1, 0.13, 0.13-3*0.26,0.13+3*0.26) 
    coef2 = r.RooRealVar("coef%d"%2,"coef%d"%2, 0.31, 0.31-3*0.49, 0.31+3*0.49) 
    coef3 = r.RooRealVar("coef%d"%3,"coef%d"%3, 0.12, 0.12-3*0.22, 0.12+3*0.22) 
    coef4 = r.RooRealVar("coef%d"%4,"coef%d"%4, 0.14, 0.14-3*0.21, 0.14+3*0.21) 
    gc.append(coef0)
    gc.append(coef1)
    gc.append(coef2)
    gc.append(coef3)
    ...
    arglist.add(coef4)
    #coef0.setConstant()
    model = r.RooBernstein("model5","model5",x,arglist)
    gc.append(model)
    return model, gc
def MKBwzredux(x):
    model_name = 'MKBwzredux'
    gc = []
    a1 = r.RooRealVar("a1", "a1", 1.544, 0, 10)
    a2 = r.RooRealVar("a2", "a2", 2.3, -100, 100)
    a3 = r.RooRealVar("a3", "a3", -0.59, -100, 100)
    for e in [a1,a2,a3]:
        gc.append(e)
        # arglist.add(e)
    a1.setConstant()
