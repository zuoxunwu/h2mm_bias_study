...
def test(x):
    ...
    #expmodel = RooGenericPdf("bwz_redux_model", "exp(@2)*(2.5)/(pow(@0-91.2,@1)+0.25*pow(2.5,@1))", RooArgList(x, a1, f))
    expmodel = RooGenericPdf("bwz_redux_bern_model", "bwz_redux_bern_model", "exp(@2)*(2.5)/(pow(@0-@3,@1)+pow(@4/2,@1))", RooArgList(x, a1, f, bwmZ, w))
    #prodmodel = RooProdPdf("prod_model","prod_model",RooArgList(linear_model,expmodel))
    return expmodel, [a1, a2, a3, f, bwmZ, w]
