...
def linear2(x):
    m2 = RooRealVar("slope2", "slope2", 0, -10, 10)        
    b2 = RooRealVar("offset2", "offset2", 0, -1000, 1000) 
    
    linear_model2 = RooGenericPdf("linear_model2", "@1*(@0-140)+@2", RooArgList(x, m2, b2))
    return linear_model2, [m2, b2]
...
def bwZreduxFixed2(x):
    a1 = RooRealVar("bwz_redux_fixed_a1", "a1", 2.0, 0.7, 2.1)
    a2 = RooRealVar("bwz_redux_fixed_a2", "a2", 0.36, 0,50)
    a3 = RooRealVar("bwz_redux_fixed_a3", "a3", -0.36, -50, 0)
    bwmZ = RooRealVar("bwz_redux_fixed_mZ","mZ",91.2,89,93)
    w = RooRealVar("bwz_redux_fixed_w","w",2.5,0,10)
...
def legendre(x): 
    L1 = RooLegendre('L1','L1',x,1,0)    
    L2 = RooLegendre('L2','L2',x,2,0)    
    coefL1 = RooRealVar("coefL1",  "coefL1", 0.5,0.,1.)
    coefL2 = RooRealVar("coefL2",  "coefL2", 0.5,0.,1.)
    params = [coefL1,coefL2]
    legendre = RooGenericPdf('legendre', "@2*@0+@3*@1", RooArgList(L1,L2,coefL1,coefL2))
    return legendre, params

def bernstein2(x, order=1): 
    #c0 = RooRealVar("c0","c0", 1.0,-1.0,1.0)
    #c1 = RooRealVar("c1","c1", 1.0,-1.0,1.0)
    #c2 = RooRealVar("c2","c2", 1.0,-1.0,1.0)
    ...
    params = []
    for i in range(0,order):
        c = RooRealVar("c"+str(i),"c"+str(i), 0.001/2**i,-0.001,0.001)
        args.add(c)
        params.append(c)
...
#----------------------------------------
# bwZRedux*(1+bernstein)
#----------------------------------------
def testtest(x):
    modelA, modelA_params = bwZreduxFixed2(x)
    modelB, modelB_params = bernstein2(x)

    #print(modelA_params)
    #modelB, modelB_params = bwZreduxFixed(x)
    #print(modelB_params)
    #params = []
    #for e in modelA_params:
#	params.append(e)
 #   for e in modelB_params:
#	params.append(e)
    
    #mix1 = RooRealVar("mix1","mix1",0.99,0,1)
    params = modelA_params + modelB_params
    prod_model = RooGenericPdf("prod","(1+@1)*@0",RooArgList(modelA,modelB))
    #params.append(mix1)
    params.append(modelA)
    params.append(modelB)
    return prod_model, params
