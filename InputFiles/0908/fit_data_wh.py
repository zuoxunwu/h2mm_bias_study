...
f = r.TFile("StackPlots_WH_2016.root")
h = f.Get("H_pair_mass_zoomH_Net_Data")
...
# plot
frame1 = x.frame(rf.Title("ttH background MC"))
dh.plotOn(frame1)
...
def MKPower(x1,order=1):
    ...
    for i in range(order):
        coef_power=r.RooRealVar("coef_power%d"%i,"coef_power%d"%i,0.1,0,1)
...
model3, gc3 = MKLegendre(x1,order=[1,2])
model4, gc4 = MKExp(x1)
model5, gc5 = MKBernstein(x,order=2)
...
pad1.cd()
# fit 
model1.fitTo(dh, rf.Range("left,right"))
model1.plotOn(frame1, rf.LineColor(r.kBlue), rf.Name("MKBwz"), rf.Range("Full"))
#model1.paramOn(frame1)
m1chi = frame1.chiSquare()
model2.fitTo(dh, rf.Range("left,right"))
model2.plotOn(frame1, rf.LineColor(r.kMagenta), rf.Name("MKPower"), rf.Range("Full"))
m2chi = frame1.chiSquare()
#model2.paramOn(frame1)
model3.fitTo(dh, rf.Range("left,right"))
model3.plotOn(frame1, rf.LineColor(r.kGreen), rf.Name("MKLegendre"), rf.Range("Full"))
m3chi = frame1.chiSquare()
model4.fitTo(dh, rf.Range("left,right"))
model4.plotOn(frame1,rf.LineColor(r.kRed), rf.Name("MKExp"), rf.Range("Full"))
m4chi = frame1.chiSquare()
#model4.paramOn(frame1)
model5.fitTo(dh, rf.Range("left,right"))
model5.plotOn(frame1,rf.LineColor(r.kYellow), rf.Name("MKBernstein"), rf.Range("Full"))
m5chi = frame1.chiSquare()
frame1.Draw()
legend = r.TLegend(0.7,0.7,0.9,0.9)
legend.AddEntry(frame1.findObject("MKBwz"),"bwZ (Z peak fixed)")
legend.AddEntry(frame1.findObject("MKPower"),"Power Law (order 1)")
legend.AddEntry(frame1.findObject("MKLegendre"),"Legendre (order 2)")
legend.AddEntry(frame1.findObject("MKExp"),"Exponential (order 1)")
